...
class Graficas:
    def __init__(self, base_dir='optimizacion'):
        self.base_dir = base_dir
        self.data_dir = os.path.join(base_dir, 'xlsx')
        self.graficas_dir = os.path.join(base_dir, 'graficas')
        if not os.path.exists(self.graficas_dir):
            os.makedirs(self.graficas_dir)
    
    def graficar_demanda_vs_capacidad(self, demanda, capacidad, titulo, etapa, numero):
        ...
        filename = f'Gpy{numero}_etapa{etapa}.png'
        plt.savefig(os.path.join(self.graficas_dir, filename))
        plt.close()

    def graficar_diferencia(self, demanda, capacidad, titulo, etapa, numero):
...
def run_completo():
    ...
    graficas.graficar_demanda_vs_capacidad(demanda_etapa1, capacidad_etapa1, 'Capacidad vs. Demanda (Etapa 1)', 1, 1)
    graficas.graficar_diferencia(demanda_etapa1, capacidad_etapa1, 'Demanda - Capacidad (Etapa 1)', 1, 2)
    ...
    graficas.graficar_demanda_vs_capacidad(demanda_etapa2, capacidad_etapa2, 'Capacidad vs. Demanda (Etapa 2)', 2, 1)
    graficas.graficar_diferencia(demanda_etapa2, capacidad_etapa2, 'Demanda - Capacidad (Etapa 2)', 2, 2)
